...
The switching engine and configuration transactions are unchanged. Hosted machine lifecycle and failed-upgrade recovery checks passed for the candidate identified in the validation record; the final CI candidate also passed interactive UAC migration and ordinary-user launch. The design and evidence are documented in [Installer](../tools/Installer/README.md) and [Validation](VALIDATION-3.4.1.md).
