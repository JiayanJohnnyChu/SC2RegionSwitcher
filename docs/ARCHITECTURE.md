...
The switching engine and configuration transactions are unchanged. Version 3.4.2 adds six licensing/source files to the MSI through separate file-keyed components; the existing application and registration components retain their identities. The installation and recovery results for 3.4.1 remain historical evidence. The design and evidence are documented in [Installer](../tools/Installer/README.md) and [Validation](VALIDATION-3.4.1.md).
