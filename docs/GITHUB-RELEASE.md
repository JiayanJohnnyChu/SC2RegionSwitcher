...
The [repository](https://github.com/JiayanJohnnyChu/SC2RegionSwitcher) is private. The active version is 3.4.1; the original 3.4.0 candidate is retained as an unpublished draft. Outstanding acceptance work is documented in [current status](HANDOFF.md).
...
5. Candidate acceptance requires machine installation lifecycle tests against those files, including upgrade recovery and old-preview detection, together with basic launch and bilingual UI checks. The completed 3.4.0 full UI/online round remains historical coverage; wider application tests are repeated only when changed behavior requires them. Completed and blocked checks are recorded separately under ignored `artifacts/` directories.
...
Promotion requires acceptance and a matching tag at the tested source commit, such as `v3.4.1-preview.1`. A tag alone does not create a release. The **Promote tested candidate to draft prerelease** workflow takes the following inputs:
