...
5. 针对这些文件测试全机安装生命周期，包括升级恢复和旧预览版检测，并进行基本启动与双语界面检查。已完成的 3.4.0 完整界面／在线测试保留为历史覆盖，仅在行为变更确有需要时扩大应用重测。完成项与受阻项分别记录于 Git 忽略的 `artifacts/`。
...
3.4.1 是首个全机包。此前的当前用户预览版需一次性卸载重装，保留 UpgradeCode 不代表可以跨上下文升级。参阅[安装器说明](../tools/Installer/README.zh-CN.md)。
