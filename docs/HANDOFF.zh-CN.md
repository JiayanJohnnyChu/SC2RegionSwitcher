...
全机包本地检查已通过：编译零警告、零错误，52 项回归、包验证、八项发布拒绝情形，以及零错误、零警告的完整 ICE 套件。[CI 运行 35721803273](https://github.com/JiayanJohnnyChu/SC2RegionSwitcher/actions/runs/35721803273) 的候选通过[运行 35722639637](https://github.com/JiayanJohnnyChu/SC2RegionSwitcher/actions/runs/35722639637)的托管全机生命周期测试。此前当前用户包的恢复结果，包括管理员对照通过，不能证明新包通过。确切证据及待测项见 [3.4.1 验证](VALIDATION-3.4.1.zh-CN.md)。

## 剩余验收工作

1. 最终文档修订需要标识明确的 CI 制品；已记录的生命周期结果适用于验证记录中指定的较早候选。
2. 交互式 UAC 安装、普通用户启动及基本双语界面检查仍待完成。已有完整 DPI 与在线往返保留为历史覆盖，重测范围仅在受影响的应用行为需要时扩大。
