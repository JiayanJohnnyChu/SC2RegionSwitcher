...
The administrator-required MSI has passed build, package and hosted lifecycle checks for the candidate identified in the validation record. The final CI candidate passed interactive UAC migration and subsequent ordinary-user launch. Basic English and Simplified Chinese Settings UI checks passed with isolated configuration. This historical preview was distributed privately.
...
The original 3.4.0 files remain unchanged. Packages are unsigned and require Microsoft .NET 10 Desktop Runtime x64. The repository was private during the 3.4.1 evaluation. The original 3.4.1 package did not include a project license; licensing and distribution materials were added in 3.4.2. Installation and migration steps are in [Setup](SETUP.md).
