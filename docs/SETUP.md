...
The public download is [3.4.2-preview.1](https://github.com/JiayanJohnnyChu/SC2RegionSwitcher/releases/tag/v3.4.2-preview.1). The 3.5.1 source has no published release package. This guide covers both; the Global language selector and the newer settings labels belong to the development version.

| Version | Interface languages | Game languages |
| --- | --- | --- |
| Public download | English and Simplified Chinese | China: Simplified Chinese; Global: English |
| Development version | Eleven interface languages | China: Simplified Chinese; Global: a selected installed text/speech pair |
...
| --- | --- |
| `SC2Switcher-3.4.2-x64.msi` | Installation requires the switcher to be closed and administrator approval. It installs for all Windows users and creates a **SC2 Region Switcher** Start menu entry. The application runs with ordinary user permissions. |
| `SC2Switcher-3.4.2-win-x64-preview.zip` | Complete extraction into a separate folder provides the portable application. Its entry point is `SC2Switcher.Wpf.exe`. It creates no shortcut or Windows uninstall entry. |
...
China requires both Simplified Chinese text and speech. The public version requires both English resources for Global; the development version requires both resources for the selected Global language. Resources are installed through Battle.net. A language shown in the switcher does not prove that an ongoing download has finished.
...
In the development interface, the file field is under **Advanced · shared game settings**. The public interface calls this section **Advanced options**. Each location can be selected with its folder or file button or entered as a full path; typed paths do not expand expressions such as `%USERPROFILE%`.
...
In the development interface, the action is disabled when the observed region, text language and speech language already match the target. A required recovery or unknown state is handled separately.
...
The development version's **Global game language (experimental)** field selects one language for both text and speech. It lists languages with both resources declared by the installation. Additional resources require installation through Battle.net.
