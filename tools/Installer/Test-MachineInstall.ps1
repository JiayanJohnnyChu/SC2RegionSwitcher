...
function Set-UpgradeCode($Database,[string]$Code){$view=$Database.OpenView('SELECT * FROM `Upgrade`');try{[void]$view.Execute();while($record=$view.Fetch()){try{[void]$view.Modify(6,$record);$record.StringData(1)=$Code;[void]$view.Modify(1,$record)}finally{[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($record)}}}finally{[void]$view.Close();[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($view)}}
...
function New-Fixture([string]$Role,[string]$Version,[string]$Token){
    ...
    try{
        ...
        Set-UpgradeCode $opened.Database $upgrade
        Set-Value $opened.Database Upgrade VersionMin ActionProperty OLDPRODUCTS '3.4.0'
        Set-Value $opened.Database Upgrade VersionMax ActionProperty OLDPRODUCTS $Version
        Set-Value $opened.Database Upgrade VersionMin ActionProperty NEWERPRODUCTS $Version
