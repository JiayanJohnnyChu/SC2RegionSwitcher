...
`_Validation.idt` 包含本构建器使用的十七张表的标准列约束。原有十五张表的子集从 WiX 3.14.1.8722 生成的对照包中导出。AppSearch 和 RegLocator 的七行约束依据标准 MSI 定义补入，并经不屏蔽规则的 ICE 套件检查。文件不包含应用标识、本地路径、载荷或测试用户数据。
